...
def reset_state():
    with open("../../game/current_state.txt") as fid:
        text=fid.read()

    state.board=[int(_) for _ in text.split()]
    ...
    for i in range(children_field.getCount()):
        ...
        name=node.getDef()
        if name == "RED_STICK" or name == "BLUE_STICK":
            nodes.append(node)

    print(f"Found {len(nodes)} sticks")
    for node in nodes:
        node.remove()
...
def reset_robot_positions():
    print("resetting robot positions")
    robots = {}
    for i in range(children_field.getCount()):
        node = children_field.getMFNode(i)
...
while robot.step(timestep) != -1:
    ...
    if not key_down and key==ord(str(1)):
        reset_robot_positions()
        filename = f"images/current_board.png"
        camera.saveImage(filename, 100)  # 100 is quality  
        print("<<<<<<<Sending>>>>>>")          
        emitter1.send(state.board.__repr__())
        key_down=True

    elif not key_down and key==ord(str(2)):
        reset_robot_positions()
        filename = f"images/current_board.png"
        camera.saveImage(filename, 100)  # 100 is quality            
        print("<<<<<<<Sending>>>>>>")          
        ...
        key_down=True
    elif not key_down and key==ord('R'):
        reset_robot_positions()
