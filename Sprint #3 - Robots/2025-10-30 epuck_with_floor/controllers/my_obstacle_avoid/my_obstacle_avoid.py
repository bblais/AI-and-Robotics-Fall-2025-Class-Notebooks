...
right_threshold = [500]*4
left_threshold = [500]*4
...
dist_left_sensors = [robot.getDevice('ps' + str(x)) for x in range(num_left_dist_sensors)]  # distance sensors
list(map((lambda s: s.enable(timeStep)), dist_left_sensors))  # Enable all distance sensors

dist_right_sensors = [robot.getDevice('ps' + str(x)) for x in range(num_right_dist_sensors,8)]  # distance sensors
